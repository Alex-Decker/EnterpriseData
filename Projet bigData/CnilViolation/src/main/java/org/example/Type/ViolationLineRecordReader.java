...
@Slf4j
@NoArgsConstructor
public class ViolationLineRecordReader extends RecordReader<ViolationLongWritable, ViolationText> {
    ...
    private ViolationLongWritable key;
    private ViolationText value;
    private boolean isCompressedInput;
    private Decompressor decompressor;
    private byte[] recordDelimiterBytes;

    public ViolationLineRecordReader(byte[] recordDelimiter) {
    ...
    public void initialize(InputSplit genericSplit, TaskAttemptContext context) throws IOException {
        ...
        if (this.start != 0L) {
            this.start += (long)this.in.readLine(new ViolationText(), 0, this.maxBytesToConsume(this.start));
        }
    ...
    public boolean nextKeyValue() throws IOException {
        if (this.key == null) {
            this.key = new ViolationLongWritable();
        }

        this.key.set(this.pos);
        if (this.value == null) {
            this.value = new ViolationText();
        }
    ...
    public ViolationLongWritable getCurrentKey() {
        return this.key;
    }

    public ViolationText getCurrentValue() {
        return this.value;
    }
